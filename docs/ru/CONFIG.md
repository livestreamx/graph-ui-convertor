...
- `index_refresh_interval_seconds`: интервал фоновой пересборки индекса каталога в секундах.
  Значение `0` отключает периодическое обновление. По умолчанию `300` (5 минут).
...
- Catalog UI поддерживает локализацию `en`/`ru` через query-параметр `lang`
  (`/catalog?lang=ru`) с сохранением выбора в cookie `cjm_catalog_ui_lang`.
  Если `lang` не задан, используется cookie, затем `Accept-Language`.
...
- В деталях услуги есть две карточки действий в одной строке: `Диаграмма уровня блоков` и
  `Диаграмма уровня процедур`. В каждой карточке доступны `Show graph`, `Open Excalidraw`
  и оба варианта скачивания.
- В `Диаграмма уровня блоков` кнопка `Show graph` открывает полноэкранный интерактивный
  граф блоков (zoom/pan/drag). Данные графа извлекаются из того же Excalidraw payload, который
  используется для отрисовки диаграммы (`/api/scenes/{scene_id}`), и доступны через
  `/api/scenes/{scene_id}/block-graph`.
  API возвращает только узлы, которые участвуют в отображаемых рёбрах.
  Если есть рёбра `block_graph`, они имеют приоритет, а рёбра `branch` игнорируются.
- `Диаграмма уровня процедур` строится on-demand для текущей услуги через тот же путь
  team-graph билдера. Режим по умолчанию: потенциальные merge-узлы текущей разметки
  относительно всех разметок той же команды:
  `merge_nodes_all_markups=true`, `merge_selected_markups=false`, `merge_node_min_chain_size=1`.
  API: `/api/scenes/{scene_id}/procedure-graph` (payload диаграммы) и
...
- На карточках каталога выводятся четыре health-маркера:
  качество графовой структуры, валидность, лучшее совпадение `procedure_id` внутри команды
  и лучшее кросс-командное пересечение. На главной странице маркеры `Графы` и `Валидность`
  по умолчанию показывают только компактный статус (`OK` или `Требует внимания`), чтобы карточки
  занимали меньше места. Маркеры `Совпадение в команде` и `Кросс-командное пересечение`
  сохраняют на карточке компактное значение лучшего overlap.
  При hover/focus у всех четырех маркеров открывается drilldown-панель.
  Для `Графы`/`Валидность` показывается breakdown по графам/блокам и пояснение проблемы,
  для overlap-маркеров - порог проблемы, top-3 похожих разметок и действие
  `Показать еще {count} разметок` для раскрытия остальных совпадений.
  Проблемные маркеры светятся по умолчанию и имеют hover-анимацию.
  Для проблем валидности, где можно точно определить блоки
  (`Несколько стартов, но нет ветвлений`, `Один и тот же блок отмечен как старт и конец`),
  в карточке показываются конкретные `block_id`, сгруппированные по проблеме.
  Если настроен `CJM_CATALOG__BLOCK_LINK_PATH`, каждый блок отображается ссылкой во внешнюю систему.
  Для проблем валидности UI явно показывает триггер для первых двух кейсов:
  либо в процедуре несколько стартов, blocks с ветвлением = 0 и эти старты не сходятся в
  block graph, либо blocks с ветвлением = 0 и нет end-блоков, завершающих граф. Последовательные
  старты и старты, которые сходятся в один downstream-block, считаются валидными. `postpone` и
  возвраты в родителя через `branches -> end` не считаются завершением. Кейс, где в одной
  процедуре один и тот же блок отмечен и как start, и как end, показывается без отдельного
  пояснения.
- На главной добавлен фильтр `Маркеры проблем` с вариантами:
  `Все`, `Типы графов`, `Валидность графов`, `Совпадение в команде`, `Кросс-командное пересечение`.
  При выборе конкретного маркера показываются только карточки, у которых проблемный именно этот маркер.
- В плашках активных фильтров добавлен кликабельный `×`, который снимает только конкретный
  фильтр и сохраняет остальные активные фильтры и поисковые токены.
...
- На desktop список каталога по-прежнему ограничен тремя карточками в строке, но если в группе
  только одна или две карточки, они растягиваются на всю ширину строки без пустых колонок.
- Поиск на главной странице каталога работает через токены в основном поле.
  ...
  Токены объединяются по логике `AND`, а каждый токен ищется по title/tags/метаданным разметки,
  а также по `procedure_id` и `block_id`.
...
- Step 3 (`Объедините графы`) теперь запускает асинхронную server-side job вместо ожидания одного
  длинного HTML-запроса. Карточка статуса на Step 3 сохраняется для состояний `running`, `ready`
  и `failed`, показывает точную причину ошибки прямо в интерфейсе и автоматически опрашивает
  сервер до завершения merge. Это убирает потерю обратной связи при долгих объединениях и
  таймаутах браузера/прокси.
- Успешные merge-job кэшируются в памяти по параметрам запроса и сигнатуре индекса каталога.
  Step 4 / Step 5, `/catalog/teams/graph/open`, `/api/teams/graph` и `/api/teams/graph-view`
  переиспользуют готовый результат через `job_id`, поэтому открытие/скачивание диаграмм не
  запускает повторную тяжелую сборку того же кросс-командного графа.
- На Step 3 после Merge остаются только статус мержинга и агрегированные счетчики
  (`N teams · M markups merged`). Подробная аналитика вынесена на Step 4.
- На Step 4 (`Analyze graphs`) рендерится dashboard из трех секций:
  `Graphs info` (распределение `markup_type`, уникальные графы, уникальные процедуры, покрытие bot/multi),
  `Entity integrity` (внутренние/внешние пересечения услуг, расщепления, доля целевого состояния)
  ...
  В `Top overloaded entities` (в UI: `Топ перегруженных разметок`) детализация выводит те же колонки по процедурам в порядке графа и
...
- Step 5 (`Get diagram`) содержит действия открытия/скачивания диаграмм по процедурам и услугам.
  В карточке уровня услуг также есть `Render graph` (`Отрисовать граф`): он открывает браузерный
  интерактивный граф услуг (уникальные графы услуг как узлы и их связи) через
  `/api/teams/graph-view` (`graph_level=service`).
