...
- В сборщике кросс-командного графа детали выбора доступны через подсказку рядом с Merge; процедуры
  подсвечиваются по услугам, пересечения выделяются светло-красным.
- В сборщике кросс-командного графа есть секция Flags. `merge_nodes_all_markups` включает расчет
  узлов слияния по всем доступным разметкам, при этом рисуются только выбранные команды.
