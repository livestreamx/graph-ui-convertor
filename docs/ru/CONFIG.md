...
- На карточках каталога выводятся четыре health-маркера:
  ...
  Для проблем валидности UI явно показывает триггер:
  либо blocks с ветвлением = 0 при стартовых блоках > 1, либо blocks с ветвлением = 0 и нет
  end-блоков кроме postpone (postpone-окончания не считаются завершением), либо в одной
  процедуре один и тот же блок отмечен и как start, и как end.
