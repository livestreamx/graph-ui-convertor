...
- На Step 3 после Merge рендерится dashboard из трех секций:
  ...
  В `Top overloaded entities` детализация выводит те же колонки по процедурам в порядке графа.
...
- В блоке `External team overlaps` на уровне команды показываются три счетчика: `total`,
  `external → selected` (внешняя команда зависит от выбранных команд) и `selected → external`
  (выбранные команды зависят от внешней команды). Сумма двух направлений всегда равна `total`.
  По кнопке `More info` раскрываются пересечения по внешним услугам с отдельными счетчиками.
