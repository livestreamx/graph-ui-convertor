...
- Для кросс-командных диаграмм есть отдельная страница: можно выбрать несколько команд и открыть
  общий граф процедур на основе `procedure_graph` (`/catalog/teams/graph`, `/api/teams/graph`,
  `team_ids` поддерживает значения через запятую).
