...
- В сборщике кросс-командного графа детали выбора доступны через подсказку рядом с Build graphs;
  процедуры подсвечиваются по услугам, пересечения выделяются светло-красным.
- В сборщике кросс-командного графа есть секция Flags. `merge_nodes_all_markups` фиксирует выбор
  учета всех разметок для узлов слияния (пока только UI).
