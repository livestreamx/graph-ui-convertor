...
- В блоке `External team overlaps` на уровне команды показывается `total` (сумма пересечений),
  а по кнопке `More info` раскрываются пересечения по внешним услугам с отдельными счетчиками.
