...
- На карточках каталога выводятся четыре health-маркера:
  ...
  либо blocks с ветвлением = 0 при стартовых блоках > 1, либо blocks с ветвлением = 0 и нет
  end-блоков, завершающих граф. `postpone` и возвраты в родителя через `branches -> end`
  не считаются завершением. Кейс, где в одной процедуре один и тот же блок отмечен и как start,
  и как end, показывается без отдельного пояснения.
