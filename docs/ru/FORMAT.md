...
- `end_block_ids` – блоки с исходящей стрелкой к маркеру END. Суффиксы:
  - без суффикса: обычный END внутри текущей процедуры.
...
- `branches` – граф смежности: ключ = исходный блок, значения = целевые блоки.
  - специальная цель `end` означает "завершить текущую процедуру и вернуться в родительскую".
    Такой блок не считается настоящим END-блоком и не должен дублироваться в `end_block_ids`.
- `finedog_unit_meta.service_name` – отображаемое имя сервиса.
- `consistent` – необязательный флаг валидности из инструмента разметки. Значение `false`
  означает, что в Catalog health разметка считается невалидной, потому что часть ключевых блоков
  потеряна, и разметку нужно актуализировать вручную через инструмент разметки.
...
- `service_name` выводится как композитный заголовок над графом в формате
  `[markup_type] Название сервиса`.
- Детерминированный лейаут: grid по процедурам, слева направо, сверху вниз.
- Для кросс-командных графов (`procedure_graph` с `is_intersection`) добавляется красный блок
  "Узлы слияния": заголовки групп в формате
  `> [Команда] [Тип разметки] Услуга x [Команда] [Тип разметки] Услуга:`, далее строки
  `(N) Название процедуры`. Узлы слияния подсвечиваются красным пунктирным овалом и номером в кружке;
  в Unidraw кружок номера также рисуется пунктиром.

## Цветовая схема (теги, блоки, стрелки)

Цвета одинаковы для Excalidraw и Unidraw. Сначала описание для человека, затем точные hex.

- Теги (типы завершения): теги вида `#exit`, `#all`, `#intermediate`, `#postpone`,
  `#turn_out` (также принимаются как `::exit`, `::all` и т.д.) задают заливку END‑маркеров и
...
- Для узлов слияния в Unidraw заливка frame остается прозрачной, акцент остается на красной
  пунктирной обводке.
- Для зон в Unidraw внешняя рамка всегда черная пунктирная; заголовок зоны рисуется черным
  текстом внутри залитого прямоугольника цвета самой зоны.
