...
- `block_graph`: при наличии для диаграмм услуг связи рисуются между блоками (ветки `branches` не визуализируются); порядок процедур берётся из `block_graph`, если `procedure_graph` пуст.
