...
5. В сборщике кросс-командного графа есть секция Feature flags: каждый флаг оформлен карточкой с
...
7. На Step 4 доступны два подраздела действий: слева диаграмма по процедурам, справа верхнеуровневая
...
8. При скачивании общего графа файл получает суффикс с `team_ids` (например,
   `team-graph_alpha_beta.excalidraw`).
9. Установите `CJM_CATALOG__DIAGRAM_FORMAT=unidraw` и `CJM_CATALOG__UNIDRAW_BASE_URL`, чтобы включить режим Unidraw.
10. Откройте диаграмму в Excalidraw/Unidraw или скачайте `.excalidraw`/`.unidraw` и `markup.json` для ручного импорта или проверки.
11. Экспортируйте `.excalidraw` в `data/excalidraw_out`, затем выполните `make convert-from-ui`.
12. При старте Catalog UI кэш `excalidraw_in` очищается, чтобы сцены пересобирались на текущем коде.
13. Локальные env-переопределения (включая шаблоны ссылок) лежат в `config/catalog/env.local`.
