...
3. Optionally set `CJM_CATALOG__DIAGRAM_EXCALIDRAW_ENABLED=false` to hide `Open Excalidraw` in UI.
4. Open in Excalidraw (if enabled) or download `.excalidraw` / `Unidraw` for manual import or review.
...
6. В сборщике кросс-командного графа есть секция Feature flags: каждый флаг оформлен карточкой с
   ...
   Над карточками есть отдельный подраздел настройки merge-узлов со слайдером
   ...
   `merge_node_min_chain_size=0` полностью отключает поиск и подсветку узлов слияния.
   `merge_node_min_chain_size=1` оставляет текущее поведение.
   Значения `>1` требуют непересекающиеся цепочки из как минимум `N` подряд идущих общих процедур;
   каждая такая цепочка считается и отображается как один merge-узел-репрезентант.
   Циклы не считаются merge-цепочками, а узлы-развилки/схлопывания считаются границами и не входят
   в цепочки при `N>1`.
...
7. На Step 3 после Merge остается только статус мержинга и агрегированные счетчики
   (`N teams · M markups merged`). Подробная аналитика вынесена в Step 4.
8. На Step 4 (`Analyze graphs`) рендерится dashboard из трех секций:
   `Graphs info` (распределение `markup_type`, уникальные графы, уникальные процедуры, bot/multi coverage),
   `Entity integrity` (внутренние/внешние пересечения услуг, расщепленные графы, доля целевого состояния)
...
9. На Step 5 доступны два подраздела действий: слева диаграмма по процедурам, справа верхнеуровневая
   диаграмма по услугам. Для API/open-роутов уровень выбирается параметром `graph_level`
   (`procedure` по умолчанию, `service` для агрегации услуг). Merge-настройки
   `merge_selected_markups`, `merge_nodes_all_markups` и `merge_node_min_chain_size`
   учитываются на уровне графа процедур до агрегации.
10. При скачивании общего графа файл получает суффикс с `team_ids` (например,
   `team-graph_alpha_beta.excalidraw`).
11. При необходимости скройте кнопку открытия Excalidraw: `CJM_CATALOG__DIAGRAM_EXCALIDRAW_ENABLED=false`.
12. Откройте диаграмму в Excalidraw (если кнопка включена) или скачайте `.excalidraw`/`Unidraw` для ручного импорта или проверки.
13. Экспортируйте `.excalidraw` в `data/excalidraw_out`, затем выполните `make convert-from-ui`.
14. При старте Catalog UI кэш `excalidraw_in` очищается, чтобы сцены пересобирались на текущем коде.
15. Локальные env-переопределения (включая шаблоны ссылок) лежат в `config/catalog/env.local`.
