...
5. В сборщике кросс-командного графа есть секция Feature flags: каждый флаг оформлен карточкой с
   ...
   карточка получает легкий зеленый оттенок, а кнопка становится темной. Флаг `merge_nodes_all_markups`
   включает расчет узлов слияния по всем доступным разметкам, при этом рисуются только выбранные команды.
