...
6. На Step 3 после Merge отображается dashboard из трех секций:
   ...
   маркировкой команд, включая списки `Multi graphs` и детали в `Top linking procedures`.
   ...
   В `External team overlaps` каждая внешняя команда показывает три счетчика:
   `total`, `external → selected` (чужие команды зависят от выбранных) и `selected → external`
   (выбранные команды зависят от чужих). Сумма двух направлений всегда равна `total`.
   По кнопке `More info` раскрываются пересечения с внешними услугами и их счетчики.
